...
class Ordenacao_Insercao
{
    ...
    //Ordenação do vetor por anonascimento
    public static Jogador [] Insercao(Jogador[] Atleta, int Contador)
    {
        for (int i=1; i<Contador; i++) 
        {
            Jogador Tmp = Atleta[i].clone();
            int j = i-1;
            boolean MaiorQ=true;
            while ((j>=0) && (Atleta[j].getanoNascimento()>=Tmp.getanoNascimento() && MaiorQ))
            {
                NCompare++;
                if (Atleta[j].getanoNascimento()==Tmp.getanoNascimento() && Atleta[j].getnome().compareTo(Tmp.getnome())>0)
                {
                    NCompare++;
                    Atleta[j+1] = Atleta[j].clone();
                    NMovim++;
                    j--;
                }
                else if(Atleta[j].getanoNascimento()>Tmp.getanoNascimento())
                {
                    NCompare++;
                    Atleta[j+1] = Atleta[j].clone();
                    NMovim++;
    ...
    {
        File Dados = new File("matricula_insercao.txt");
        Dados.createNewFile();
