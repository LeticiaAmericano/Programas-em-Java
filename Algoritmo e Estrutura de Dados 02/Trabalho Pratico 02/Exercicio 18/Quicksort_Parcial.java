...
class Quicksort_Parcial
{
    ...
    //Ordenação do vetor por estadoNascimento
    public static Jogador[] Quicksort (Jogador [] Atleta, int Esq, int Dir) 
    {
        int i = Esq, 
            j = Dir-1;
        int Contador = 0;
        int Inicio = 0;
        Jogador Temp = Atleta[i].clone();
        String Pivo = Atleta[(Esq+Dir)/2].getestadoNascimento();
        while (i<=j) 
        {
            while (Atleta[i].getestadoNascimento().compareTo(Pivo)<0)
                i++;
            while (Atleta[j].getestadoNascimento().compareTo(Pivo)>0)
                j--;
            if (i <= j)
            {
                Temp = Atleta[i].clone();
        ...
        {
            NCompare++;
            Atleta = Quicksort(Atleta,Esq,j);
        }
        if (i<Dir)
        {
            NCompare++;
            Atleta = Quicksort(Atleta,i, Dir);
        }
        return Atleta;
    }
    //Desempate por nome
    public static Jogador [] Selecao(Jogador[] Atleta, int Contador)
    {
        for (int i=0; i<Contador-1; i++) 
        {
            ...
            {
                NCompare++;
                if ((Atleta[Menor].getestadoNascimento().compareTo(Atleta[j].getestadoNascimento()) == 0)&& (Atleta[Menor].getnome().compareTo(Atleta[j].getnome())>0))
                {
                    Tmp = Atleta[j].clone();
    ...
    {
        File Dados = new File("matricula_quicksort.txt");
        Dados.createNewFile();
