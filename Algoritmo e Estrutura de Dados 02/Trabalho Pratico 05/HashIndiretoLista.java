class  HashIndiretoLista {
   ...
   booleano  pesquisando ( int  elemento ) {
      pos int = h (elemento);
      retornar tabela [pos] . pesquisar (elemento);
   }
...
/ *
   removedor vazio (elemento interno) {
      int resp = NULO;
      if (pesquisar (elemento) == false) {
         lançar uma nova exceção ("Erro ao remover!");
      } outro {
         pos int = h (elemento);
